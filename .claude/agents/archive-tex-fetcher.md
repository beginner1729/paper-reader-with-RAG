...
Expected input (passed as natural language in the task prompt):
- archive_url: URL to arXiv abs/pdf/e-print or a .zip/.tar/.tar.gz/.tgz archive.

Expected output (return paths in your response):
- paper_dir: Directory containing extracted TeX sources.
- tex_notes_dir: Directory containing per-TeX file notes.
- tex_manifest_path: Markdown manifest summarizing all TeX files.
